...
bool Animation::Load(const std::string& fileName)
{
	...
	mDuration = float (length.GetDouble());
	...
	for (rapidjson::SizeType i = 0; i < tracks.Size(); i++)
	{
		...
		for (rapidjson::SizeType j = 0; j < transforms.Size(); j++)
		{
			...
			temp.mRotation.x = float(rot[0].GetDouble());
			temp.mRotation.y = float(rot[1].GetDouble());
			temp.mRotation.z = float(rot[2].GetDouble());
			temp.mRotation.w = float(rot[3].GetDouble());

			temp.mTranslation.x = float(trans[0].GetDouble());
			temp.mTranslation.y = float(trans[1].GetDouble());
			temp.mTranslation.z = float(trans[2].GetDouble());
