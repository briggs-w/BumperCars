...
bool Skeleton::Load(const std::string& fileName)
{
	...
	for (rapidjson::SizeType i = 0; i < count; i++)
	{
		...
		temp.mLocalBindPose.mRotation.x = float(rot[0].GetDouble());
		temp.mLocalBindPose.mRotation.y = float(rot[1].GetDouble());
		temp.mLocalBindPose.mRotation.z = float(rot[2].GetDouble());
		temp.mLocalBindPose.mRotation.w = float(rot[3].GetDouble());

		temp.mLocalBindPose.mTranslation.x = float(trans[0].GetDouble());
		temp.mLocalBindPose.mTranslation.y = float(trans[1].GetDouble());
		temp.mLocalBindPose.mTranslation.z = float(trans[2].GetDouble());
